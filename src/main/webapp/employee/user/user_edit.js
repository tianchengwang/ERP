...
function doFindObjectById(id){
	var url="doFindUserObjectById.do"
	var params={"id":id};
	$.post(url,params,function(result){
		if(result.state==1){
		 doInitEditFormData(result.data.object);	
		}else{
...
function doInitEditFormData(obj){
	...
	$("#u_mobile").val(obj.u_mobile);
	$("#u_address").val(obj.u_address);
	$("#u_valid").val(obj.u_valid);
...
function doSaveOrUpdate(){
	...
	//获得模态框上绑定的id值
	var id=$("#modal-dialog").data("id");
	if(id)params.u_id=id;//动态添加属性(修改时需要id的值)
	//根据id是否有值来判定是修改还是添加
	console.log(JSON.stringify(params));
	//return;
	var updateUrl="doUpdateUserObject.do";
	var insertUrl="register.do";
