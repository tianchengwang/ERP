...
function setTableBodyRows(result){//result 为一个json对象
	...
	for(var i in result){//循环一次取一行[{"name":"AAA",...},{},{}]
		var tr=$("<tr></tr>");
		tr.data("id",result[i].u_id);//key/value
		var firstTd='<td><input type="checkbox" name="checkedItem" value="[id]"></td>';
		firstTd=firstTd.replace("[id]",result[i].u_id);
		tr.append(firstTd);	
		
		tr.append("<td>"+result[i].u_username+"</td>");
		tr.append("<td>"+result[i].u_password+"</td>");
		tr.append("<td>"+result[i].u_email+"</td>");
		tr.append("<td>"+result[i].u_mobile+"</td>");
		tr.append("<td>"+result[i].u_address+"</td>");
		tr.append("<td>"+result[i].u_valid+"</td>");
