$(document).ready(function(){
	...
	$('.rightinfo')
	.on("click",".btn-delete",DeleteMessage);
	//初始化页面数据
	doGetObjects();
...
function doGetObjects(){
	...
	var url="doFindQuestionMObjects.do"
...
function setTableBodyRows(result){//result 为一个json对象
	...
	for(var i in result){//循环一次取一行[{"name":"AAA",...},{},{}]
		...
		var tr=$("<tr></tr>");
		//在tr对象上绑定一个值
		tr.data("id",result[i].m_id);//key/value
		//tr对象中追加td字符串对象
		var firstTd='<td><input type="checkbox" name="checkedItem" value="[id]"></td>';
		//将firstTd字符串中的[id]替换为一个具体指
		firstTd=firstTd.replace("[id]",result[i].m_id);
		tr.append(firstTd);	
		
		tr.append('<td>'+result[i].m_id+'</td>');		
		tr.append("<td>"+result[i].m_name+"</td>");
		tr.append("<td>"+result[i].m_question+"</td>");
		tr.append("<td>"+result[i].m_answer+"</td>");
		tr.append('<td><input type="button" class="scbtn  btn-update" style="width:50px;height:30px;" value="修改"/> <input type="button" class="scbtn  btn-delete" style="width:50px;height:30px;" value="删除"/></td>');
...
function DeleteMessage(){
	//定义一个url(对应服务端控制器中的一个方法)
	var flag = confirm("确定删除这条消息吗？");
	if(flag==false)
		return;
	var id = $(this).parent().parent().data("id");
	var url="deleteMessageObject.do"
	var params = {"id":id};
    $.ajaxSettings.async = true;
    $.post(url,params,function(result){//result为一个json对象(JsonResult)
    	if(result.state==1){//成功
