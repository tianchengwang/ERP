...
function doFindObjectById(id){
	var url="findSalaryObjectById.do"
	var params={"id":id};
...
function doSaveOrUpdate(){
	...
	//根据id是否有值来判定是修改还是添加
	var updateUrl="doUpdateSalaryObject.do";
	var insertUrl="doSaveSalaryObject.do";
...
function getEditFormData(){
 //1.获得页面上用户输入的数据,封装为json对象(相对比较灵活)
  var params={//根据id获得数据
	...
	"basicssalary":$("#basicssalary").val(),
	"ranksalary":$("#ranksalary").val(),
	"achievements":$("#achievements").val(),
	"mealsalary":$("#mealsalary").val(),
	"housesalary":$("#housesalary").val(),
	"travelexpenses":$("#travelexpenses").val(),
	"note":$("#noteId").val()
  }
