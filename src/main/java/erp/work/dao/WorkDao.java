...
public interface WorkDao extends BaseDao<Work> {
	 ...
	 * @param 注解进行声明
	 * */
	List<Work> findObjects(@Param("id")Integer employeeid,@Param("startIndex")Integer startIndex,@Param("pageSize")Integer pageSize);
	
	/**根据id执行查询操作
	 * @param id 来自页面上的某条记录的id
	 * */
	Work findObjectById(@Param("id")Integer employeeid,@Param("month")Integer month);
